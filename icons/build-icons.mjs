...
/** Filled disc used as the base of every badge-style icon. */
const disc = (fill, ring) =>
  `<circle cx="64" cy="64" r="56" fill="${ring}"/><circle cx="64" cy="64" r="48" fill="${fill}"/>`;

const ICONS = {
  // --- currency ---------------------------------------------------------
  coin: svg(
    `${disc(C.brand, C.brandDark)}` +
  ...
  wallet: svg(
    `<rect x="12" y="30" width="104" height="72" rx="14" fill="${C.brandDark}"/>` +
      `<rect x="12" y="30" width="104" height="60" rx="14" fill="${C.brand}"/>` +
  ...
  // --- rewards ----------------------------------------------------------
  gift: svg(
    `<rect x="14" y="46" width="100" height="22" rx="6" fill="${C.brand}"/>` +
      ...
      `<ellipse cx="44" cy="32" rx="18" ry="14" fill="${C.brand}"/>` +
      `<ellipse cx="84" cy="32" rx="18" ry="14" fill="${C.brand}"/>` +
  ...
  minus: svg(
    `${disc(C.danger, C.dangerDark)}` + `<rect x="34" y="56" width="60" height="16" rx="8" fill="${C.white}"/>`,
  ),

  // --- people and access ------------------------------------------------
  user: svg(
    `${disc(C.neutral, C.neutralDark)}` +
  ...
  staff: svg(
    `<path d="M64 10l44 18v34c0 28-19 47-44 56-25-9-44-28-44-56V28z" fill="${C.infoDark}"/>` +
      `<path d="M64 22l33 13v27c0 21-14 35-33 43-19-8-33-22-33-43V35z" fill="${C.info}"/>` +
  ...
  code: svg(
    `<circle cx="64" cy="40" r="30" fill="${C.brand}"/>` +
      `<circle cx="64" cy="40" r="12" fill="none" stroke="${C.brandInk}" stroke-width="10"/>` +
      `<rect x="55" y="62" width="18" height="54" rx="6" fill="${C.brand}"/>` +
